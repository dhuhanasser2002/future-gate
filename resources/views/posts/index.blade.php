...
<section class="light">
    ...
    <a href="{{ route('posts.create') }}" class="btn btn-primary btn-md mb-5 mt-5">Add Post</a>
            ...
            <div class="postcard__text t-dark">
               ...
               <div>
                ...
                <ul class="postcard__tagbox">
                    ...
                    <li class="tag__item">
                    <a class="" href="{{ route('posts.edit', $post->id) }}">update</i></a>
                    </li>
                    @endif
                    @if (auth()->check() && auth()->user()->can('delete', $post))
                    <li class="tag__item">
                    <form action="{{ route('posts.force', $post->id) }}" method="POST" style="display:inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="dropdown-item" class="btn btn-secondary" onclick="return confirm('Are you sure you want to delete this post?')">Delete</button>
                    </form>
                    @endif
                    </li>
                    @if (auth()->check() && auth()->user()->can('delete', $post))
                    <li class="tag__item play {{ $cardClass }}">
                    <form action="{{ route('posts.destroy', $post->id) }}" method="POST" style="display:inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="dropdown-item" onclick="return confirm('Are you sure you want to archived this post?')">Add To Archive</button>
                    </form>
                    </li>
                    @endif
                </ul>
